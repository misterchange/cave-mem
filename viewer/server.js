#!/usr/bin/env node
/**
 * stoneage Memory Stream Viewer — server (SQLite edition)
 *
 * Serves the real-time memory stream UI and SSE endpoint.
 * Reads from ~/.claude/stoneage-memory.db (Node 22 built-in SQLite).
 * Polls DB every 500ms and pushes new entries to SSE clients.
 *
 * Usage:
 *   node C:/Nitin/Nitins/stoneage/viewer/server.js
...
const server = http.createServer((req, res) => {
  ...
  if (url === '/handoff' || url === '/handoff.md' || url === '/handoff.txt') {
    ...
    md += `5. Check \`${MEM_DB}\` via the stoneage viewer for live updates\n\n`;
    ...
    try {
      ...
      let outName = 'stoneage-handoff.md';
      if (sessionFilter) {
        ...
        outName = `stoneage-handoff-${slug}.md`;
      }
  ...
  if (url === '/sessions') {
    ...
    const out = rows.map(r => {
      ...
      return {
        ...
        handoff_file: path.join(claudeDir, `stoneage-handoff-${info.slug}.md`),
      };
    });
    res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  if (url === '/handoff-path') {
    const claudeDir = path.dirname(MEM_DB);
    const filePath  = path.join(claudeDir, 'stoneage-handoff.md');
...
server.listen(PORT, '127.0.0.1', () => {
  console.log(`stoneage memory stream viewer`);
