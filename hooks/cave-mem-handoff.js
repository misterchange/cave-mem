#!/usr/bin/env node
/**
 * cave-mem — Shared handoff generator
 *
 ...
 *   • hooks/cave-mem-observer.js  (write-on-every-tool-call)
 *
 * Exports:
...
const { claudeDir } = require('./cave-mem-config');
const { MEM_DB }    = require('./cave-mem-db');
...
function writeAllHandoffs(db, opts = {}) {
  ...
  if (!onlySession) {
    ...
    fs.writeFileSync(path.join(claudeDir, 'cave-mem-handoff.md'), combined, 'utf8');
  }
  ...
  for (const [sid, info] of labels) {
    ...
    fs.writeFileSync(
      path.join(claudeDir, `cave-mem-handoff-${info.slug}.md`),
      md, 'utf8'
    );
  }

  return { labels, combined: path.join(claudeDir, 'cave-mem-handoff.md') };
}
